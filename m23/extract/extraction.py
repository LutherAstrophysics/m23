...
def extractStars(imageData, referenceLogFileName, saveAs, radiusOfExtraction=5):
    ...
    with open(saveAs, "w") as fd:
        fd.write(f"Stars Found: {len(list(filter(lambda starFlux: starFlux[0] > 0, starsFluxes)))}\n")
...
def starsPositionsInLogFile(fileName):
    with open(fileName, "r") as fd:
        ...
        return [np.array(line.split()[:2][::-1], dtype="float16") for line in allLines]
...
def circleMatrix(radius):
    ...
    for row in range(-radius, radius + 1):
        for col in range(-radius, radius + 1):
            if math.sqrt((row) ** 2 + (col) ** 2) <= radius:
                myMatrix[row + radius][col + radius] = 1
    return myMatrix


filePath = r"C:\Data Processing\RefImage\ref_revised_71.txt"
